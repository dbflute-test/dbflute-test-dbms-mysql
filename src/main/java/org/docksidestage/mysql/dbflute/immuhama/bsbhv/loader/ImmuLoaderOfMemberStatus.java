...
public class ImmuLoaderOfMemberStatus {
    ...
    //                                                                       Load Referrer
    //                                                                       =============
    protected List<ImmuMember> _referrerMember;
     ...
     * });
     * for (ImmuMemberStatus memberStatus : <span style="color: #553000">memberStatusList</span>) {
     ...
     */
    public NestedReferrerLoaderGateway<ImmuLoaderOfMember> loadMember(ReferrerConditionSetupper<ImmuMemberCB> refCBLambda) {
        myBhv().loadMember(_selectedList, refCBLambda).withNestedReferrer(refLs -> _referrerMember = refLs);
        return hd -> hd.handle(new ImmuLoaderOfMember().ready(_referrerMember, _selector));
    }
