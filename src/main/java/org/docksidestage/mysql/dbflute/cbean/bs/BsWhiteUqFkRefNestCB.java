...
public class BsWhiteUqFkRefNestCB extends AbstractConditionBean {
     ...
     */
    public void union(UnionQuery<WhiteUqFkRefNestCB> unionCBLambda) {
     ...
     */
    public void unionAll(UnionQuery<WhiteUqFkRefNestCB> unionCBLambda) {
        final WhiteUqFkRefNestCB cb = new WhiteUqFkRefNestCB(); cb.xsetupForUnion(this); xsyncUQ(cb);
        try { lock(); unionCBLambda.query(cb); } finally { unlock(); } xsaveUCB(cb);
     ...
     * }).alwaysPresent(<span style="color: #553000">whiteUqFkRefNest</span> <span style="color: #90226C; font-weight: bold"><span style="font-size: 120%">-</span>&gt;</span> {
     *     ... = <span style="color: #553000">whiteUqFkRefNest</span>.<span style="color: #CC4747">getWhiteUqFkRef()</span>; <span style="color: #3F7E5E">// you can get by using SetupSelect</span>
     * });
     * </pre>
    ...
    public WhiteUqFkRefNss setupSelect_WhiteUqFkRef() {
        ...
        doSetupSelect(() -> query().queryWhiteUqFkRef());
    ...
    public static class HpSpecification extends HpAbstractSpecification<WhiteUqFkRefNestCQ> {
        ...
        public WhiteUqFkRefCB.HpSpecification specifyWhiteUqFkRef() {
            ...
            if (_whiteUqFkRef == null) {
                _whiteUqFkRef = new WhiteUqFkRefCB.HpSpecification(_baseCB
                    , xcreateSpQyCall(() -> _qyCall.has() && _qyCall.qy().hasConditionQueryWhiteUqFkRef()
                                    , () -> _qyCall.qy().queryWhiteUqFkRef())
                    , _purpose, _dbmetaProvider, xgetSDRFnFc());
                if (xhasSyncQyCall()) { // inherits it
                    _whiteUqFkRef.xsetSyncQyCall(xcreateSpQyCall(
                        () -> xsyncQyCall().has() && xsyncQyCall().qy().hasConditionQueryWhiteUqFkRef()
                      , () -> xsyncQyCall().qy().queryWhiteUqFkRef()));
                }
            }
        ...
        public org.dbflute.cbean.chelper.dbms.HpSDRFunctionMySql<WhiteUqFkRefNestCB, WhiteUqFkRefNestCQ> myselfDerived() {
            ...
            return cHSDRF(_baseCB, _qyCall.qy(), (String fn, SubQuery<WhiteUqFkRefNestCB> sq, WhiteUqFkRefNestCQ cq, String al, DerivedReferrerOption op)
