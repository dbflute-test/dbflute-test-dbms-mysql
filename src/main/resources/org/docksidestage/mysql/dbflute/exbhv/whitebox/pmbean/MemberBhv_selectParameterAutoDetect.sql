...
select member.MEMBER_ID
 ...
 /*BEGIN*/where
   ...
   and member.MEMBER_STATUS_CODE = /*pmb.basicCls:cls(MemberStatus)|comment(second option test)|ref(MEMBER.MEMBER_STATUS_CODE)*/'PRV'
   and member.MEMBER_STATUS_CODE = /*pmb.fixedCls:cls(MemberStatus.Withdrawal)|comment(fixed classification)*/'PRV'
   and member.MEMBER_STATUS_CODE = /*pmb.memberStatusCode:cls(MemberStatus)|ref(MEMBER)*/'PRV'
   ...
   and member.MEMBER_STATUS_CODE in /*pmb.basicClsList:cls(MemberStatus)*/('FML', 'PRV')
   and member.MEMBER_STATUS_CODE in /*pmb.fixedClsOneList:cls(MemberStatus.Formalized)*/('WDL')
   and member.MEMBER_STATUS_CODE in /*pmb.fixedClsPluralList:cls(MemberStatus.Formalized, Withdrawal)*/('FML', 'PRV')
