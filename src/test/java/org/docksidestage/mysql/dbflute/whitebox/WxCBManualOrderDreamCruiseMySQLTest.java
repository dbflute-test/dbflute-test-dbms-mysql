...
public class WxCBManualOrderDreamCruiseMySQLTest extends UnitContainerTestCase {
    ...
    public void test_DreamCruise_ManualOrder_CaseWhen_SpecifyCalculation_freedom() throws Exception {
        ...
        cb.query().addOrderBy_MemberId_Asc().withManualOrder(op -> {
            ...
            SpecifiedColumn versionNo = dreamCruiseCB.specify().columnVersionNo();
            op.when_GreaterEqual(servicePointCount.multiply(versionNo.plus(1)).convert(cv -> cv.round(2)));
    ...
    public void test_DreamCruise_ManualOrder_pluralColumn() throws Exception {
        ...
        String sql = cb.toDisplaySql();
        log(ln() + sql);
        String exp = "(dfloc.MEMBER_ID * dfrel_27.SERVICE_POINT_COUNT) * dfrel_26.REMINDER_USE_COUNT";
    ...
    public void test_DreamCruise_ManualOrder_union_journeyLogBook_basic() throws Exception {
        ...
        assertContains(sql, "order by MEMBER_ID * SERVICE_POINT_COUNT_27 asc");
        assertEquals(2, Srl.count(sql, ", dfrel_27.MEMBER_SERVICE_ID as MEMBER_SERVICE_ID_27"));
        assertEquals(2, Srl.count(sql, "left outer join MEMBER_SERVICE dfrel_27 on dfloc.MEMBER_ID = dfrel_27.MEMBER_ID"));
    }
    ...
    public void test_DreamCruise_ManualOrder_union_journeyLogBook_nested() throws Exception {
        ...
        log(ln() + sql);
        assertContains(sql, "union");
        assertContains(sql, "order by (MEMBER_ID * VERSION_NO) * SERVICE_POINT_COUNT_27 asc");
        assertEquals(2, Srl.count(sql, ", dfrel_27.MEMBER_SERVICE_ID as MEMBER_SERVICE_ID_27"));
        assertEquals(2, Srl.count(sql, "left outer join MEMBER_SERVICE dfrel_27 on dfloc.MEMBER_ID = dfrel_27.MEMBER_ID"));
    }
    ...
    public void test_DreamCruise_ManualOrder_convert_both() throws Exception {
        ...
        cb.query().addOrderBy_MemberId_Asc().withManualOrder(op -> {
            ...
            op.multiply(columnPoint.convert(cv -> cv.coalesce(2)));
