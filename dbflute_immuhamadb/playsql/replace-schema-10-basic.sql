...
CREATE TABLE CDEF_MEMBER_STATUS
(
...
) COMMENT = '[区分値]商品カテゴリ : 商品のカテゴリを表現するマスタ。自己参照FKの階層になっている。';


CREATE TABLE CDEF_PRODUCT_STATUS
(
...
) COMMENT = '[区分値]商品ステータス : 商品のステータスを表現する固定的なマスタ。';


CREATE TABLE CDEF_REGION
(
...
) COMMENT = '[区分値]地域 : 主に会員の住所に対応する漠然とした地域。かなりざっくりした感じではある。唯一の業務的one-to-oneの親テーブルのケース。';


CREATE TABLE CDEF_SERVICE_RANK
(
...
) COMMENT = '[区分値]サービスランク : 会員のサービスレベルを表現するランク。(ゴールドとかプラチナとか)';


CREATE TABLE CDEF_WITHDRAWAL_REASON
(
...
) COMMENT = '[区分値]退会理由 : 会員に選ばせる定型的な退会理由のマスタ。そういえば、これ表示順カラムがないですねぇ...';


CREATE TABLE MEMBER
(
	MEMBER_ID INT NOT NULL AUTO_INCREMENT COMMENT '会員ID : 連番として自動採番される。会員IDだけに限らず採番方法はDBMS次第。',
	MEMBER_REGISTER_DATETIME DATETIME NOT NULL COMMENT '会員登録日時',
...
CREATE TABLE MEMBER_LOGIN_PASSWORD
(
	MEMBER_LOGIN_PASSWORD_ID BIGINT NOT NULL AUTO_INCREMENT COMMENT '会員ログインパスワードID',
	...
	UPDATE_DATETIME DATETIME NOT NULL COMMENT '更新日時',
	UPDATE_USER VARCHAR(200) NOT NULL COMMENT '更新ユーザ',
	PRIMARY KEY (MEMBER_LOGIN_PASSWORD_ID)
) COMMENT = '会員ログインパスワード : 会員とは one-to-one で、会員一人につき必ず一つのセキュリティ情報がある';


CREATE TABLE MEMBER_PASSWORD_REMINDER
(
	MEMBER_PASSWORD_REMINDER_ID BIGINT NOT NULL AUTO_INCREMENT COMMENT '会員パスワードリマインダID',
	MEMBER_ID INT NOT NULL COMMENT '会員ID : そのまま one-to-one を構成するためのFKとなる。',
	...
	UPDATE_DATETIME DATETIME NOT NULL COMMENT '更新日時',
	UPDATE_USER VARCHAR(200) NOT NULL COMMENT '更新ユーザ',
	PRIMARY KEY (MEMBER_PASSWORD_REMINDER_ID)
) COMMENT = '会員パスワードリマインダ : 会員とは one-to-one で、会員一人につき必ず一つのセキュリティ情報がある';


CREATE TABLE MEMBER_PROFILE
(
	MEMBER_PROFILE_ID BIGINT NOT NULL AUTO_INCREMENT COMMENT '会員プロフィールID',
	MEMBER_ID INT NOT NULL COMMENT '会員ID : 連番として自動採番される。会員IDだけに限らず採番方法はDBMS次第。',
...
) COMMENT = '会員プロフィール : 会員のプロフィールやアカウントなどの基本情報を保持する。基本的に物理削除はなく、退会したらステータスが退会会員になる。ライフサイクルやカテゴリの違う会員情報は、one-to-oneなどの関連テーブルにて。';
...
(
	MEMBER_STATUS_ID BIGINT NOT NULL AUTO_INCREMENT COMMENT '会員ステータスID',
	MEMBER_ID INT NOT NULL COMMENT '会員ID : 連番として自動採番される。会員IDだけに限らず採番方法はDBMS次第。',
	MEMBER_STATUS_CODE CHAR(3) NOT NULL COMMENT '会員ステータスコード : 会員ステータスを識別するコード。固定的なデータなので連番とか番号にはせず、データを直接見たときも人が直感的にわかるように、例えば "FML" とかの３桁のコード形式にしている。(3って何会員だっけ？とかの問答をやりたくないので)',
	MEMBER_STATUS_UPDATE_DATETIME DATETIME NOT NULL COMMENT '会員ステータス更新日時',
	REGISTER_DATETIME DATETIME NOT NULL COMMENT '登録日時 : レコードが登録された日時',
	REGISTER_USER VARCHAR(200) NOT NULL COMMENT '登録ユーザー : レコードを登録したユーザー',
	UPDATE_DATETIME DATETIME NOT NULL COMMENT '更新日時',
	UPDATE_USER VARCHAR(200) NOT NULL COMMENT '更新ユーザ',
	PRIMARY KEY (MEMBER_STATUS_ID)
) COMMENT = '会員ステータス : 会員のプロフィールやアカウントなどの基本情報を保持する。基本的に物理削除はなく、退会したらステータスが退会会員になる。ライフサイクルやカテゴリの違う会員情報は、one-to-oneなどの関連テーブルにて。';


CREATE TABLE MEMBER_WITHDRAWAL
(
	MEMBER_ID INT NOT NULL COMMENT '会員ID : 連番として自動採番される。会員IDだけに限らず採番方法はDBMS次第。',
	WITHDRAWAL_REASON_CODE CHAR(3) NOT NULL COMMENT '退会理由コード : 定型的な退会した理由を参照するコード。何も言わずに退会する会員もいるので必須項目ではない。',
...
(
	PRODUCT_DETAIL_ID BIGINT NOT NULL AUTO_INCREMENT COMMENT '商品詳細ID',
...
(
	PRODUCT_PRICE_ID BIGINT NOT NULL AUTO_INCREMENT COMMENT '商品価格ID',
	PRODUCT_ID INT NOT NULL COMMENT '商品ID',
...
(
	...
	PRODUCT_DETAIL_ID BIGINT NOT NULL COMMENT '商品詳細ID',
	PRODUCT_PRICE_ID BIGINT NOT NULL COMMENT '商品価格ID',
	...
	CONSTRAINT UQ_PURCHASE UNIQUE (MEMBER_ID, PURCHASE_DATETIME)
...
(
	...
	UPDATE_USER VARCHAR(200) NOT NULL COMMENT '更新ユーザ',
	PRIMARY KEY (PURCHASE_ID)
) COMMENT = '購入ステータス : 一つの商品に対する購入を表現する(購入履歴とも言える)。実業務であれば購入詳細というテーブルを作り、「購入という行為」と「その中身（詳細）」を違う粒度のデータとしてそれぞれ管理するのが一般的と言えるでしょう。というか、注文とか請求とかそういうことを考え始めたらもっと複雑になるはずですが、ExampleDBということで割り切っています。';
...
CREATE INDEX IX_PRODUCT_PRODUCT_NAME USING BTREE ON PRODUCT_DETAIL (PRODUCT_NAME ASC);
CREATE INDEX IX_PURCHASE_MEMBER_ID USING BTREE ON PURCHASE (MEMBER_ID ASC);
CREATE INDEX IX_PURCHASE_PRODUCT_ID USING BTREE ON PURCHASE (PRODUCT_DETAIL_ID ASC);
CREATE INDEX IX_PURCHASE_PRODUCT_DATETIME USING BTREE ON PURCHASE (PURCHASE_DATETIME ASC);
...
ALTER TABLE MEMBER_LOGIN
	...
	REFERENCES CDEF_MEMBER_STATUS (MEMBER_STATUS_CODE)
	ON UPDATE NO ACTION
	ON DELETE NO ACTION
;
...
ALTER TABLE MEMBER_STATUS
	ADD CONSTRAINT FK_MEMBER_STATUS_XX_MEMBER_STATUS FOREIGN KEY (MEMBER_STATUS_CODE)
	REFERENCES CDEF_MEMBER_STATUS (MEMBER_STATUS_CODE)
	ON UPDATE RESTRICT
	ON DELETE RESTRICT
;
...
ALTER TABLE CDEF_PRODUCT_CATEGORY
	ADD CONSTRAINT FK_PRODUCT_CATEGORY_PARENT FOREIGN KEY (PARENT_CATEGORY_CODE)
	REFERENCES CDEF_PRODUCT_CATEGORY (PRODUCT_CATEGORY_CODE)
	ON UPDATE NO ACTION
	ON DELETE NO ACTION
;
...
ALTER TABLE PRODUCT_DETAIL
	ADD CONSTRAINT FK_EVENT_PRODUCT_PRODUCT_CATEGORY FOREIGN KEY (PRODUCT_CATEGORY_CODE)
	REFERENCES CDEF_PRODUCT_CATEGORY (PRODUCT_CATEGORY_CODE)
	ON UPDATE RESTRICT
	ON DELETE RESTRICT
;
...
ALTER TABLE PRODUCT_DETAIL
	ADD CONSTRAINT FK_EVENT_PRODUCT_PRODUCT_STATUS FOREIGN KEY (PRODUCT_STATUS_CODE)
	REFERENCES CDEF_PRODUCT_STATUS (PRODUCT_STATUS_CODE)
	ON UPDATE RESTRICT
	ON DELETE RESTRICT
;
...
ALTER TABLE MEMBER_ADDRESS
	...
	REFERENCES CDEF_REGION (REGION_ID)
	ON UPDATE NO ACTION
	ON DELETE NO ACTION
;
...
ALTER TABLE MEMBER_SERVICE
	...
	REFERENCES CDEF_SERVICE_RANK (SERVICE_RANK_CODE)
	ON UPDATE NO ACTION
	ON DELETE NO ACTION
;
...
ALTER TABLE MEMBER_WITHDRAWAL
	...
	REFERENCES CDEF_WITHDRAWAL_REASON (WITHDRAWAL_REASON_CODE)
	ON UPDATE NO ACTION
	ON DELETE NO ACTION
;
...
ALTER TABLE MEMBER_LOGIN_PASSWORD
...
ALTER TABLE MEMBER_PASSWORD_REMINDER
	ADD CONSTRAINT FK_MEMBER_PASSWORD_REMINDER_MEMBER FOREIGN KEY (MEMBER_ID)
	REFERENCES MEMBER (MEMBER_ID)
	ON UPDATE RESTRICT
	ON DELETE RESTRICT
;
...
ALTER TABLE MEMBER_PROFILE
	ADD CONSTRAINT FK_EVENT_MEMBER_MEMBER FOREIGN KEY (MEMBER_ID)
	REFERENCES MEMBER (MEMBER_ID)
	ON UPDATE RESTRICT
	ON DELETE RESTRICT
;
...
ALTER TABLE MEMBER_STATUS
	ADD CONSTRAINT FK_MEMBER_STATUS_XX_MEMBER FOREIGN KEY (MEMBER_ID)
	REFERENCES MEMBER (MEMBER_ID)
	ON UPDATE RESTRICT
	ON DELETE RESTRICT
;
...
ALTER TABLE PRODUCT_DETAIL
	ADD CONSTRAINT FK_EVENT_PRODUCT_PRODUCT FOREIGN KEY (PRODUCT_ID)
...
ALTER TABLE PURCHASE
	ADD CONSTRAINT FK_PURCHASE_PRODUCT_DETAIL FOREIGN KEY (PRODUCT_DETAIL_ID)
...
ALTER TABLE PURCHASE
	ADD CONSTRAINT FK_PURCHASE_PRODUCT_PRICE FOREIGN KEY (PRODUCT_PRICE_ID)
...
ALTER TABLE PURCHASE_STATUS
	ADD CONSTRAINT FK_PURCHASE_STATUS_PURCHASE FOREIGN KEY (PURCHASE_ID)
	REFERENCES PURCHASE (PURCHASE_ID)
	ON UPDATE RESTRICT
	ON DELETE RESTRICT
;
